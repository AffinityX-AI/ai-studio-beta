...
export default function SeoCopilot() {
  ...
  const handleDeleteThread = (thread: any) => {
    axios.delete(`/api/seo-copilot/threads/${thread.id}`).then((res) => {
  ...
  useEffect(() => {
    if (currentThread?.status === 'running') {
      const interval = setInterval(() => {
        axios(`/api/seo-copilot/threads/${currentThread.id}/status`).then(
          (res: any) => {
    ...
    >
      ...
      >
        <div className='flex flex-col items-center justify-center w-full py-4 px-7 pb-4 border-b border-dotted border-gray-200'>
          ...
          <p className='text-emerald-800 opacity-40 uppercase font-bold text-[11px] my-0 tracking-widest'>
            SMB SEO Copilot
          </p>
        </div>
        <ThreadsList
